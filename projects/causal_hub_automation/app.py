    ...
    def add_iv(fig):
        if t_start and t_end:
            fig.add_vrect(x0=str(t_start.date()), x1=str(t_end.date()),
                          fillcolor="rgba(123,127,255,0.14)", line_width=0)
    ...
    def annotate_period_avgs(fig, col, pre_df, post_df, color, t_start, t_end, df):
        ...
        if len(pre_df) and col in pre_df.columns:
            ...
            fig.add_annotation(x=str(mid_pre.date()), y=pre_avg,
                ...
                bgcolor="rgba(17,17,38,0.92)", borderpad=2)
    ...
    # ── Correlation Heatmap ───────────────────────────────────────────────────
        ...
        if len(conv_sess_cols) >= 2:
            ...
            fig.add_trace(go.Heatmap(
                ...
                colorscale=[[0,"#DC2626"],[0.5,"#F3F4F6"],[1,"#84CC16"]],
        ...
        if col and col in pre_df.columns and t_start and t_end and len(post_df) > 0:
            ...
            if col in post_df.columns:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    fill="toself", fillcolor="rgba(123,127,255,0.20)",
...
def call_llm_narrator(ci_summary, context):
    ...
    try:
        ...
        if not api_key: return None
        client = OpenAI(base_url=ARK_BASE_URL, api_key=api_key)
        hyp_type = context.get("hypothesis_type", "incrementality")
        # Hypothesis-specific framing instructions
        _hyp_instructions = {
            "cannibalisation": (
                "This is a CANNIBALISATION test. The client fears TikTok is stealing volume from other channels. "
                "Frame the result in terms of: did total conversions grow AND did non-TikTok conversions hold/grow? "
                "A positive result means TikTok GREW the pie, not just took a slice. A negative result or flat non-TikTok means cannibalisation cannot be ruled out."
            ),
            "incrementality": (
                "This is an INCREMENTALITY test. Frame the result as: did TikTok drive conversions that would not have happened otherwise? "
                "A statistically significant positive lift proves incremental value."
            ),
            "brand_lift": (
                "This is a BRAND LIFT test. Frame the result in terms of awareness, consideration, or search lift. "
                "Emphasise the upper-funnel business value even if conversion lift is modest."
            ),
            "cross_channel_halo": (
                "This is a CROSS-CHANNEL HALO test. Frame the result as: did TikTok exposure lift performance in other channels (e.g. Google Search, Direct)? "
                "Emphasise the multiplier effect across the media mix."
            ),
            "launch_effect": (
                "This is a LAUNCH EFFECT test. Frame the result as: did the TikTok campaign accelerate adoption/awareness of the new product/service? "
                "Emphasise the step-change vs the pre-launch baseline."
            ),
        }
        framing = _hyp_instructions.get(hyp_type, _hyp_instructions["incrementality"])
        two_model = context.get("two_model_recommendation", {})
        two_model_note = ""
        if two_model.get("needed"):
            two_model_note = f"\nNote: This is model run '{context.get('model_run_label', 'Model 1')}'. The full analysis requires two model runs: {two_model.get('rationale','')}"
        system = f"""You are a senior TikTok measurement analyst writing a client-facing results narrative.
...
Hypothesis framing: {framing}
Output ONLY valid JSON — no markdown, no preamble.
Schema: {{"headline":"Bold one-sentence result that directly addresses the hypothesis","what_happened":"2-3 sentences on what the model found — be specific about numbers","business_meaning":"2 sentences on what this means commercially for the client","methodology_note":"1 sentence on model approach for credibility","caveat":"1 sentence on limitations if any (or null)"}}"""
        user = f"""Advertiser: {context.get('advertiser')} | Hypothesis type: {hyp_type} | Hypothesis: {context.get('hypothesis')}
...
Cross-channel confounders accounted for: {context.get('confounders_included', 'none noted')}{two_model_note}
...
def call_synthesis_llm(api_key, endpoint, stats_text, brief_text, past_cases=""):
    """
    ...
    past_cases_block = f"\nRELEVANT PAST CASES (from knowledge base — use for pattern recognition only):\n{past_cases}\n" if past_cases else ""

    prompt = f"""You are a senior TikTok Measurement Partner analyst specialising in causal inference for digital advertising. You have received a sales brief and raw data statistics. Your job is to synthesise these into a structured pre-modelling analysis — exactly as an experienced analyst would do before touching a model.
...
{{
  "hypothesis_type": "One of: cannibalisation | incrementality | brand_lift | cross_channel_halo | launch_effect | holdout_validation | custom",
  "analytical_risk": "One of: low | medium | high | very_high — assess based on confounders, pre-period length, and simultaneous events",
  "analytical_risk_reason": "One sentence explaining the risk rating. Be specific — name the channels, events, or data issues driving it.",
  ...
  "cross_channel_confounders": [
    {{
      "channel": "Channel name (e.g. Facebook, Google Ads, Snapchat)",
      "description": "Describe what this channel did using sessions/conversions from the data — e.g. 'Meta sessions +43% pre→post'. Do NOT fabricate spend figures.",
      "confounder_risk": "low | medium | high | critical",
      "must_include_as_covariate": true,
      "rationale": "Why this channel confounds the TikTok attribution if not controlled for."
    }}
  ],
  "promo_events": [
    {{
      "name": "Event name (e.g. New Season Sale, BFCM, End of Year)",
      "start": "YYYY-MM-DD",
      "end": "YYYY-MM-DD",
      "sits_in": "pre | post | both",
      "impact": "low | medium | high | critical",
      "flag_recommended": true,
      "rationale": "How this event affects the counterfactual and what to do about it."
    }}
  ],
  "flag_suggestions": [
    {{
      "name": "FlagName_snake_case",
      "start": "YYYY-MM-DD",
      "end": "YYYY-MM-DD",
      "is_treatment": false,
      "source": "brief",
      "rationale": "Why this period needs a flag — cite the specific event or anomaly."
    }}
  ],
  "two_model_recommendation": {{
    "needed": true,
    "model_1_target": "Column name or description for first model target (e.g. TotalConversions)",
    "model_1_label": "Short label (e.g. Total lift)",
    "model_2_target": "Column name or description for second model target (e.g. NonCatalogConversions)",
    "model_2_label": "Short label (e.g. Cannibalisation check)",
    "rationale": "One sentence explaining why two runs are needed and what each proves."
  }},
  "recommended_periods": {{
  ...
  "covariate_recommendations": [
    {{"column": "column_name", "role": "Covariate", "rationale": "Why this variable works as a covariate — cite R² from the stats or channel name from brief.", "risk": "None|Low|Medium|High"}},
  ...
  "brief_gaps": ["Things missing from the brief that would strengthen the analysis — e.g. 'Daily Facebook spend not in data set'"],
  "readiness": "Ready|Caution|Not ready",
  "readiness_reason": "One sentence."
}}

IMPORTANT RULES:
- cross_channel_confounders: if the brief mentions any other paid channels scaling simultaneously, ALL of them must appear here with must_include_as_covariate=true. A parallel spend ramp across Facebook/Google/Snapchat at the same time as the TikTok intervention is the most common reason causal models over-attribute lift to TikTok. For the description field, use session or conversion figures from the actual dataset stats — NEVER fabricate spend dollar amounts that are not in the data.
- flag_suggestions: include ALL promotional events and structural breaks from the brief as flags. Set is_treatment=true ONLY for the TikTok intervention window itself.
- two_model_recommendation: set needed=true for cannibalisation hypotheses (one model for total, one for non-treatment channel). Also set to true when the brief asks to prove two separate claims.
- analytical_risk: "very_high" when multiple channels scaled simultaneously AND a major promotional event sits inside the post-period."""
...
if st.session_state.step == 0:
    st.markdown(f'''<div style="text-align:center;padding:32px 0 24px">
      ...
      <div style="font-size:14px;color:{C['grey']};max-width:480px;margin:0 auto">Paste the brief and upload your data. The AI will read both and derive the hypothesis before you touch a model.</div>
    ...
    if st.session_state.raw_df is not None:
        ...
        with btn_c:
            ...
            if st.button("Analyse", use_container_width=True, type="primary"):
                # Aggregate TTAM if needed (use default L4 map + broad conv events)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Lazy AI synthesis — runs once here so dates are pre-filled ─────────────
    if st.session_state.ai_synthesis is None:
        with st.spinner("AI reading brief and data…"):
            _stats_c2 = build_synthesis_stats(df_cfg, st.session_state.brief_text)
            _hyp_hint_c2 = ""
            if st.session_state.brief_text:
                _bt_c2 = st.session_state.brief_text.lower()
                if "cannibal" in _bt_c2: _hyp_hint_c2 = "cannibalisation"
                elif "brand" in _bt_c2 or "awareness" in _bt_c2: _hyp_hint_c2 = "brand_lift"
                elif "launch" in _bt_c2 or "new product" in _bt_c2: _hyp_hint_c2 = "launch_effect"
                else: _hyp_hint_c2 = "incrementality"
            _past_c2 = _build_past_cases_context(_hyp_hint_c2, n=3)
            _syn_c2, _err_c2 = call_synthesis_llm(
                st.session_state.ark_key, st.session_state.ark_endpoint,
                _stats_c2, st.session_state.brief_text, past_cases=_past_c2,
            )
            if _syn_c2:
                st.session_state.ai_synthesis = _syn_c2
                _rp_c2 = _syn_c2.get("recommended_periods", {})
                if _rp_c2 and _rp_c2.get("pre_start") and st.session_state.pre_start is None:
                    try:
                        st.session_state.pre_start  = pd.Timestamp(_rp_c2["pre_start"]).date()
                        st.session_state.pre_end    = pd.Timestamp(_rp_c2["pre_end"]).date()
                        st.session_state.post_start = pd.Timestamp(_rp_c2["post_start"]).date()
                        st.session_state.post_end   = pd.Timestamp(_rp_c2["post_end"]).date()
                    except Exception:
                        pass
                st.rerun()

    # ── Seed defaults: synthesis-suggested dates → data-driven fallback ────────
    def _cfg_defaults():
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(f'<div style="color:{C["grey"]};margin-bottom:16px;font-size:13px">Review the data before analysis. AI synthesis runs in the background so your periods are pre-filled when you reach Configure.</div>', unsafe_allow_html=True)

    df = parse_dates(st.session_state.daily_df.copy())

    # ── Run AI synthesis lazily here so dates are ready for Configure ──────────
    if st.session_state.ai_synthesis is None:
        with st.spinner("AI reading brief and data…"):
            _stats_q = build_synthesis_stats(df, st.session_state.brief_text)
            _hyp_hint_q = ""
            if st.session_state.brief_text:
                _bt_q = st.session_state.brief_text.lower()
                if "cannibal" in _bt_q: _hyp_hint_q = "cannibalisation"
                elif "brand" in _bt_q or "awareness" in _bt_q: _hyp_hint_q = "brand_lift"
                elif "launch" in _bt_q or "new product" in _bt_q: _hyp_hint_q = "launch_effect"
                else: _hyp_hint_q = "incrementality"
            _past_q = _build_past_cases_context(_hyp_hint_q, n=3)
            _syn_q, _err_q = call_synthesis_llm(
                st.session_state.ark_key, st.session_state.ark_endpoint,
                _stats_q, st.session_state.brief_text, past_cases=_past_q,
            )
            if _syn_q:
                st.session_state.ai_synthesis = _syn_q
                # Seed pre/post dates from synthesis (once, if not already set)
                _rp_q = _syn_q.get("recommended_periods", {})
                if _rp_q and _rp_q.get("pre_start") and st.session_state.pre_start is None:
                    try:
                        st.session_state.pre_start  = pd.Timestamp(_rp_q["pre_start"]).date()
                        st.session_state.pre_end    = pd.Timestamp(_rp_q["pre_end"]).date()
                        st.session_state.post_start = pd.Timestamp(_rp_q["post_start"]).date()
                        st.session_state.post_end   = pd.Timestamp(_rp_q["post_end"]).date()
                    except Exception:
                        pass
                st.rerun()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.session_state.file_type == "sot" and st.session_state.daily_df is not None:
        ...
        CH_ORDER  = ["Direct","Paid_Social_TikTok","Paid_Search_Google","Paid_Social_Meta","Organic_Search"]
        CH_COLORS = {"Direct":C["purple"],"Paid_Social_TikTok":"#00D8FF","Paid_Search_Google":"#FF7070","Paid_Social_Meta":"#5BA8FF","Organic_Search":"#4AE882","Organic":"#4AE882"}
        ...
        # ── DATA QUALITY (period-aware) ────────────────────────────────────────────
        with st.expander("📋 Data Quality", expanded=False):
            _sot_sigs = quality_checks(df)
            _sot_highs = [s for s in _sot_sigs if s["sev"]=="high"]
            _sot_conv_c = next((c for c in conv_cols_sot if "TikTok" not in c), conv_cols_sot[0] if conv_cols_sot else None)
            _sot_total = int(df[_sot_conv_c].sum()) if _sot_conv_c and _sot_conv_c in df.columns else 0
            _sot_dr = f"{df['p_date'].min().strftime('%d %b')}–{df['p_date'].max().strftime('%d %b %Y')}"
            st.markdown(
                f'<div class="mrow">'
                f'{mtile("Date Range", _sot_dr)}'
                f'{mtile("Days", str(len(df)), "mvc" if len(df)>=60 else "mva" if len(df)>=40 else "mvr")}'
                f'{mtile("Issues", str(len(_sot_highs)), "mvr" if _sot_highs else "mvc")}'
                f'</div>', unsafe_allow_html=True)
            for _sq in _sot_sigs:
            ...
            # Calendar gaps
            _sot_full = pd.date_range(df["p_date"].min(), df["p_date"].max(), freq="D")
            _sot_gaps = [d for d in _sot_full if d not in df["p_date"].values]
            if _sot_gaps:
                st.markdown(f'<div class="card card-r">{pill("GAPS","pill-r")} {len(_sot_gaps)} missing date(s): {", ".join(d.strftime("%d %b") for d in _sot_gaps[:8])}</div>', unsafe_allow_html=True)
            # Pre:post ratio
            if t_start is not None and t_end is not None:
                _sot_pre_n = len(df[df["p_date"] < t_start])
                _sot_post_n = len(df[(df["p_date"] >= t_start) & (df["p_date"] <= t_end)])
                if _sot_post_n > 0:
                    _sot_rat = _sot_pre_n / _sot_post_n
                    _sot_cls = "card-c" if _sot_rat >= 2 else "card-a" if _sot_rat >= 1.5 else "card-r"
                    st.markdown(f'<div class="card {_sot_cls}" style="margin-top:8px"><strong>Pre/post ratio: {_sot_rat:.1f}×</strong> ({_sot_pre_n} pre · {_sot_post_n} post days)</div>', unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        _snap_pre  = df[df["p_date"] < t_start]  if t_start is not None else df
        _snap_post = df[(df["p_date"] >= t_start) & (df["p_date"] <= t_end)] if t_start is not None else pd.DataFrame()
        _snap_cards = []
        for _ch_k, _ch_cols in _snap_channels.items():
            if _ch_k == "Other": continue
            for _col in _ch_cols:
                if not any(k in _col for k in ["Conversions","Sessions","Revenue"]): continue
                _pre_avg  = _snap_pre[_col].mean()  if _col in _snap_pre.columns  and len(_snap_pre)  else 0
                _post_avg = _snap_post[_col].mean() if _col in _snap_post.columns and len(_snap_post) else 0
                ...
                _snap_cards.append((_lbl, _pre_avg, _post_avg, _lift_s, "Paid_Social_TikTok" in _col))
        if _snap_cards:
        ...
        # Period reminder strip
        _pre_s_eda  = st.session_state.pre_start
        _pre_e_eda  = st.session_state.pre_end
        _post_s_eda = st.session_state.post_start
        _post_e_eda = st.session_state.post_end
        if _pre_s_eda and _pre_e_eda and _post_s_eda and _post_e_eda:
            st.markdown(
                f'<div style="display:flex;gap:6px;align-items:center;margin-bottom:10px;flex-wrap:wrap">'
                f'<span style="font-size:11px;font-weight:600;color:{C["grey"]};letter-spacing:.06em">PERIODS</span>'
                f'<span style="background:{C["purple"]}25;border:1px solid {C["purple"]}50;border-radius:6px;padding:2px 10px;font-size:11px;color:{C["purple_l"]}">'
                f'Pre&nbsp;&nbsp;{_pre_s_eda.strftime("%d %b %Y")} – {_pre_e_eda.strftime("%d %b %Y")}</span>'
                f'<span style="background:{C["green"]}18;border:1px solid {C["green"]}40;border-radius:6px;padding:2px 10px;font-size:11px;color:{C["green"]}">'
                f'Post&nbsp;&nbsp;{_post_s_eda.strftime("%d %b %Y")} – {_post_e_eda.strftime("%d %b %Y")}</span>'
                f'</div>',
                unsafe_allow_html=True,
            )
        ...
        if ov_view == "Time Series":
            ...
            # Weekly stacked bar — conversions by channel (easier to read than daily)
            df_w = df.copy()
            df_w["week"] = df_w["p_date"].dt.to_period("W").apply(lambda r: r.start_time)
            df_grp = df_w.groupby("week").sum(numeric_only=True).reset_index()
            df_grp.rename(columns={"week": "p_date"}, inplace=True)
            col_lookup_ov2 = {"Conversions": conv_cols_sot, "Sessions": sess_cols_sot, "Revenue": rev_cols_sot}
            for ch in ov_channels:
                col_list = col_lookup_ov2.get(ov_metric, [])
                col = next((c for c in col_list if c.startswith(ch)), None)
                if col and col in df_grp.columns:
                    fig_ov.add_trace(go.Bar(
                        x=df_grp["p_date"], y=df_grp[col],
                        name=ch_lbl(ch), marker_color=ch_col(ch),
            ...
            # Horizontal bar — daily avg lift per channel, % labelled — slide-ready
            lift_rows = []
            col_lookup_ov3 = {"Conversions": conv_cols_sot, "Sessions": sess_cols_sot, "Revenue": rev_cols_sot}
            for ch in ov_channels:
                col_list = col_lookup_ov3.get(ov_metric, [])
                col = next((c for c in col_list if c.startswith(ch)), None)
                if col and col in df.columns and len(pre_df) and len(post_df):
                    ...
                    lift_rows.append({"ch": ch_lbl(ch), "abs": abs_lift, "pct": pct_lift,
                                      "pre": pre_avg, "post": post_avg, "color": ch_col(ch)})
            if lift_rows:
                lift_rows.sort(key=lambda x: x["abs"])
                _max_abs = max(abs(r["abs"]) for r in lift_rows) if lift_rows else 1
                _min_val = min(r["abs"] for r in lift_rows) if lift_rows else 0
                fig_ov.add_trace(go.Bar(
                    y=[r["ch"] for r in lift_rows],
                    x=[r["abs"] for r in lift_rows],
                    orientation="h",
                ...
                fig_ov.update_layout(**CHART,
                    ...
                    xaxis_title="Daily average change",
                    height=max(300, len(lift_rows) * 65),
                )
                _left_pad = min(_min_val * 1.8, -_max_abs * 0.05) if _min_val < 0 else -_max_abs * 0.02
                fig_ov.update_layout(
                    xaxis_range=[_left_pad, _max_abs * 1.1],
                    margin=dict(l=160, r=240, t=45, b=45),
                )
        ...
        with sot_b1:
            if st.button("Back",key="sot_eda_back"): st.session_state.step=1; st.rerun()
        with sot_b2:
            if st.button("Continue to Model →", use_container_width=True, type="primary", key="sot_eda_fwd"):
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.expander(f"Flag Variables ({len(confirmed)} confirmed)" + (" — edit before running" if not confirmed else ""), expanded=not confirmed):
        ...
        if not st.session_state.flag_vars:
            st.session_state.flag_vars = suggest_flags(_df_flags, synthesis=st.session_state.ai_synthesis)
    ...
    # ── Cross-channel confounder warning (from brief synthesis) ───────────────
    _syn6 = st.session_state.ai_synthesis or {}
    _ccf6 = _syn6.get("cross_channel_confounders", [])
    _must_covs = [cf["channel"] for cf in _ccf6 if cf.get("must_include_as_covariate")]
    if _must_covs:
        _missing_covs = [ch for ch in _must_covs if not any(ch.lower() in c.lower() for c in num_cols)]
        _present_covs = [ch for ch in _must_covs if any(ch.lower() in c.lower() for c in num_cols)]
        _warn_html = "".join(f'<div class="t-meta" style="margin-bottom:2px">· <strong>{ch}</strong></div>' for ch in _must_covs)
        _miss_note = ""
        if _missing_covs:
            _miss_note = f'<div class="t-meta" style="color:{C["red"]};margin-top:6px">⚠️ Not in dataset: {", ".join(_missing_covs)} — request spend data from the client or agency.</div>'
        st.markdown(f'''<div style="background:{C["amber"]}11;border:1px solid {C["amber"]}55;border-radius:10px;padding:12px 16px;margin-bottom:14px">
<div class="t-meta" style="font-weight:700;color:{C["amber"]};margin-bottom:6px">Brief requires these channels as covariates:</div>
{_warn_html}{_miss_note}
<div class="t-meta" style="margin-top:6px;font-style:italic">These channels scaled in parallel with TikTok — excluding them risks attributing their lift to TikTok.</div>
</div>''', unsafe_allow_html=True)
    ...
    with mc1:
        ...
        # brief-named channel matches (fuzzy — channel name appears in column name)
        brief_cov_matches = [c for c in num_cols if any(ch.lower() in c.lower() for ch in _must_covs) and c != target_sel]
        default_covs = list(dict.fromkeys(brief_cov_matches + hyp_cov_matches + [c for c in spend_cols + flag_cols if c in num_cols and c != target_sel]))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if ci_r is None:
        ...
        if st.session_state.ai_synthesis and not st.session_state.get("_kb_saved"):
            try:
                save_brief_to_knowledge_base(
                    advertiser=st.session_state.advertiser or "Unknown",
                    hypothesis_type=st.session_state.ai_synthesis.get("hypothesis_type", "incrementality"),
                    brief_text=st.session_state.brief_text or "",
                    synthesis=st.session_state.ai_synthesis,
            ...
            if st.session_state.ci_narrator is None:
                if st.button("Generate narrative", key="narrator_btn"):
                    with st.spinner("Writing client narrative..."):
                        ...
                        _syn_n = st.session_state.ai_synthesis or {}
                        _ccf_n = _syn_n.get("cross_channel_confounders", [])
                        _confounders_str = ", ".join(cf.get("channel","") for cf in _ccf_n if cf.get("must_include_as_covariate")) or "none"
                        ctx_n = {
                            "advertiser": st.session_state.advertiser,
                            "hypothesis": st.session_state.hypothesis,
                            "hypothesis_type": _syn_n.get("hypothesis_type", "incrementality"),
                            "post_start": str(st.session_state.post_start),
                            "post_end":   str(st.session_state.post_end),
                            "target":     tgt_name,
                            "confounders_included": _confounders_str,
                            "two_model_recommendation": _syn_n.get("two_model_recommendation", {}),
                        }
